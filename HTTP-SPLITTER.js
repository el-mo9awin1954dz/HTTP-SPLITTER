...
const BUGALERT = args[5];
const token = args[6];
const channel = args[7];
const messageme = args[8];
...
if(INJECTION === undefined || RESPONSE === undefined || BUGALERT === undefined || token === undefined || channel === undefined || messageme === undefined) {
...
async function check(i){
    try{
        ...
        if(JSON.stringify(response.body).indexOf(RESPONSE) !== -1){
            console.log(`${Self.data[i]} VULNERABLE TO HEADER INJECTION (HTTP splitting || HTTP smuggling).`)
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠿⣧⣤⡤⢽⣧⣤⠤⢿⢤⣴⣿⣤⣤⣤⣿⣤⣤⣼⡦⠤⠴⡶⣿⡇⠀⠀⠀⠀⠀⠀⠀
		...
		console.log('\n\n\x1b[31m',"FOUND BUG || PAYLOAD >>",'\x1b[0m',INJECTION);
                ...
                const FOUND = Self.data[i]+ " <<< " + INJECTION + " >>> ";
		...
		function sendMessage (token, channel, message) {
            ...
            console.log(`${Self.data[i]} NOT VULNERABLE TO HEADER INJECTION (HTTP splitting || HTTP smuggling).`)
        }
    }catch{
        console.log(`${Self.data[i]} NOT VULNERABLE TO HEADER INJECTION (HTTP splitting || HTTP smuggling).`)
    }
    ...
    if(Self.check_index === Self.data.length){
        ...
        if(!Self.results.length){
            console.log("NO LINKS IS VULNERABLE TO HEADER INJECTION (HTTP splitting || HTTP smuggling).")
        }else{
...
//Main
if(!Self_Args.length){
    console.log("node HTTP-SPLITTER.js TARGET-URLS.txt OUTPUT.txt INJECTION-HEADER RESPONSE-CHECK BUG-ALERT TOKEN ID-CHANNEL NOTIFICATION-MSG")
    process.exit()
}
